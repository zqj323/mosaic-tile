"""v6: Pre-diff + freeze encoder + Per-Task Router Re-Init + Weak AR loss in Phase 2.
v5 failed: router (re-initialized) was re-aligned by CE loss → sim drift.
v6 fix: add weak AR=0.05 in Phase 2 to counteract CE's alignment force.
AR is directional (only penalizes positive sim) → allows router learning, blocks collapse.
"""
...
PHASE2_AR_LAMBDA = 0.05  # Weak AR to prevent sim drift
...
def main():
    ...
    os.makedirs('/root/exp_v6_ar_weak', exist_ok=True)
    log_path = '/root/exp_v6_ar_weak/experiment.log'
    ...
    log("=" * 60)
    log("V6: Pre-diff + Freeze Encoder + Per-Task Router Re-Init + Phase2 AR=0.05")
    log(f"v5 failed: CE re-aligned router. v6: weak AR loss blocks alignment.")
    log(f"Seed={SEED}, 5 tasks x 20 classes, Phase2 AR={PHASE2_AR_LAMBDA}")
    log("=" * 60)
    ...
    # Phase 2: CL with per-task router re-init + AR loss
    log(f"\n=== PHASE 2: 5-Task CL (Per-Task Router Re-Init + AR={PHASE2_AR_LAMBDA}) ===")
    ...
    for task_id in range(5):
        ...
        log(f"  Router re-initialized. Trainable: {trainable/1e6:.1f}M. AR={PHASE2_AR_LAMBDA}")
        ...
        for epoch in range(EPOCHS_PER_TASK):
            ...
            for xb, yb in train_loader:
                ...
                combined, weights, logits, act_mat, _ = model(xb, temperature=1.0)
                ce = F.cross_entropy(combined, yb)
                ar = ar_loss_fn(logits, PHASE2_AR_LAMBDA)
                loss = ce + ar
                loss.backward(); opt.step()
                total_ce += ce.item(); total_ar += ar.item(); n_batches += 1
            if epoch % 20 == 0:
                mean_s, _ = compute_sim(model, sim_loader, dev)
                log(f"  T{task_id} E{epoch:3d} ce={total_ce/n_batches:.3f} ar={total_ar/n_batches:.4f} sim={mean_s:.3f}")
    ...
    torch.save({'model_state': model.state_dict(), 'final_results': final, 'sim': mean_s},
               '/root/exp_v6_ar_weak/final_model.pt')
    log("Saved: /root/exp_v6_ar_weak/final_model.pt")
