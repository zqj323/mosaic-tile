"""v4: Pre-diff + freeze encoder + freeze old experts + TRAINABLE router.
Hypothesis: v2 failed because frozen router (trained on 100-class distribution)
couldn't adapt to 20-class/task distribution in Phase 2.
v4: unfreeze router in Phase 2 — router can re-learn to concentrate weights on active tiles.
"""
...
class ContinualLifecycleMoE(nn.Module):
    ...
    def freeze_for_task(self, task_id):
        """v4: freeze classifier of old experts, but keep ALL routers (activation) trainable."""
        for i, expert in enumerate(self.experts):
            frozen_classifier = (i != task_id % NUM_EXPERTS)
            for param in expert.classifier.parameters():
                param.requires_grad = not frozen_classifier
            # Router (activation): always trainable in Phase 2
...
@torch.no_grad()
def compute_router_concentration(act_mat, task_id):
    """Measure how much router weight goes to the active expert (task_id % 4)."""
    active = task_id % NUM_EXPERTS
    incoming = act_mat.sum(dim=1)  # B×4
    weights = F.softmax(incoming, dim=1)
    return weights[:, active].mean().item()
...
def main():
    ...
    os.makedirs('/root/exp_v4_router_trainable', exist_ok=True)
    log_path = '/root/exp_v4_router_trainable/experiment.log'
    ...
    log("=" * 60)
    log("V4: Pre-diff + Freeze Encoder + TRAINABLE Router in Phase 2")
    log(f"Hypothesis: v2 failed because frozen router couldn't adapt to")
    log(f"per-task class distribution. v4 keeps router trainable.")
    ...
    torch.save({'model_state': {k: v.clone() for k, v in model.state_dict().items()}, 'sim': mean_s},
               '/root/exp_v4_router_trainable/prediff_ckpt.pt')

    # ── Phase 2: CL with TRAINABLE router ──
    log("\n=== PHASE 2: 5-Task CL (Freeze Encoder + Trainable Router) ===")
    model.freeze_encoder = True
    # Freeze encoder from Task 0
    for param in model.encoder.parameters():
        param.requires_grad = False
    log("  Encoder frozen from Task 0")
    log("  Router (activation) REMAINS TRAINABLE throughout Phase 2")
    opt = torch.optim.AdamW(model.parameters(), lr=1e-3, weight_decay=5e-4)
    ...
    torch.save({'model_state': model.state_dict(), 'final_results': final, 'sim': mean_s},
               '/root/exp_v4_router_trainable/final_model.pt')
    log("Saved: /root/exp_v4_router_trainable/final_model.pt")
