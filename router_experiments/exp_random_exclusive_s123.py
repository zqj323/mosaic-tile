...
def evaluate_split(model, device):
    ...
    for tid, target in enumerate(TASK_CLASSES):
        ...
        for xb, yb in loader:
            ...
            for c in target: mask |= (yb == c)
            if mask.sum() > 0:
                correct += (pred[mask] == yb[mask]).sum().item()
                total += mask.sum().item()
        results[f'task{tid}'] = correct / total * 100 if total > 0 else 0
    ft = get_full_data(train=False)
    loader = DataLoader(ft, batch_size=BATCH_SIZE, shuffle=False, num_workers=2, pin_memory=True)
    correct = total = 0
    for xb, yb in loader:
        xb, yb = xb.to(device), yb.to(device)
        # For full eval, use learned router (Phase 1 style)
