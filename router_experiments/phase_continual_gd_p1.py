...
parser.add_argument('--freeze_encoder', action='store_true', default=False,
                    help='Freeze encoder from Task 0 in CL phase')
parser.add_argument('--num_tasks', type=int, default=5,
                    help='Number of CL tasks')
parser.add_argument('--concentrated', type=int, default=0,
                    help='P1 concentrated: N primary classes get 80%% of samples (0=uniform)')
parser.add_argument('--p1_only', action='store_true', default=False,
                    help='Exit after Phase 1 pre-diff (skip CL)')
...
if args.pretrain_epochs > 0 and not pretrain_done:
    ...
    if args.concentrated > 0:
        targets = np.array(pretrain_full.targets)
        weights = np.ones(len(targets))
        for cls in range(args.concentrated):
            weights[targets == cls] = 0.8 / args.concentrated
        for cls in range(args.concentrated, 100):
            weights[targets == cls] = 0.2 / (100 - args.concentrated)
        pretrain_loader = DataLoader(pretrain_full, batch_size=BATCH_SIZE,
                                      sampler=WeightedRandomSampler(weights, len(pretrain_full), replacement=True),
    ...
    print(f"{'═'*60}\n")
    if args.p1_only:
        print(f"  P1_ONLY: exiting. Checkpoint: {pre_diff_path}")
        exit(0)
