"""v5: Pre-diff + freeze encoder + Per-Task Router Re-initialization.
Phase 2: each task resets router randomly, learns routing from scratch.
Encoder (frozen) provides stable features, router (5K params) learns in-task routing.
Hypothesis: solves both distribution shift (router fresh each task) and sim drift
(router doesn't carry Phase 1 alignment bias; CE can't re-align across 120 epochs).
"""
...
class ContinualLifecycleMoE(nn.Module):
    ...
    def reset_router(self):
        """Re-initialize all experts' activation (router) layers randomly."""
        ...
        # Reset usage/activation EMA buffers to uniform
    ...
    def freeze_for_task(self, task_id):
        """v5: freeze all classifiers, keep ALL routers trainable (per-task re-init)."""
        for i, expert in enumerate(self.experts):
            # All classifiers frozen (old experts) or trainable (current expert)
            frozen = (i != task_id % NUM_EXPERTS)
            for param in expert.classifier.parameters():
                param.requires_grad = not frozen
            # All routers trainable
            for param in expert.activation.parameters():
                param.requires_grad = True
        return task_id % NUM_EXPERTS
...
def main():
    ...
    os.makedirs('/root/exp_v5_per_task_router', exist_ok=True)
    log_path = '/root/exp_v5_per_task_router/experiment.log'
    ...
    log("=" * 60)
    log("V5: Pre-diff + Freeze Encoder + Per-Task Router Re-Init")
    log("Hypothesis: fresh router each task learns in-task routing")
    log("without carrying Phase 1 class distribution bias.")
    log(f"Seed={SEED}, 5 tasks x 20 classes")
    log("=" * 60)
    ...
    log("lambda_AR=0.1, epochs=200, kill=1")
    ...
    # Save pre-diff checkpoint with router state
    prediff_ckpt = {k: v.clone() for k, v in model.state_dict().items()}
    torch.save({'model_state': prediff_ckpt, 'sim': mean_s, 'results': results},
               '/root/exp_v5_per_task_router/prediff_ckpt.pt')

    # Phase 2: CL with per-task router re-init
    log("\n=== PHASE 2: 5-Task CL (Per-Task Router Re-Init) ===")
    log("Strategy: reset router at each task, train from scratch")
    log("Encoder + old classifiers frozen")
    ...
    for task_id in range(5):
        ...
        # Per-task router re-initialization
        model.reset_router()
        log(f"\nTASK {task_id}: classes {task_start}-{task_end-1}, active expert {task_id % 4}")
        log(f"  Router re-initialized (random)")
    ...
    torch.save({'model_state': model.state_dict(), 'final_results': final, 'sim': mean_s},
               '/root/exp_v5_per_task_router/final_model.pt')
    log("Saved: /root/exp_v5_per_task_router/final_model.pt")
