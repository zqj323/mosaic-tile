...
def get_task_ranges(num_tasks, class_offset=0):
    """Return list of (start_cls, end_cls, n_classes) for each task.
    Distributes (NUM_CLASSES - class_offset) classes as evenly as possible across num_tasks."""
...
# ── Task setup ──
def get_task_data(task_id, train=True, num_tasks=5, class_offset=0):
    """Get CIFAR-100 subset for a specific task."""
    ...
    ranges = get_task_ranges(num_tasks, class_offset)
...
@torch.no_grad()
def evaluate_all_tasks(model, device, num_tasks=5, class_offset=0):
    """Evaluate accuracy on all tasks + combined."""
    model.eval()
    results = {}
...
# ── CL Baseline helpers (EWC / LwF) ──
def compute_fisher(model, loader, device, num_batches=50):
    ...
    for xb, yb in loader:
        ...
        model.zero_grad()
        combined, _, _, _, _ = model(xb.to(device))
        loss = F.cross_entropy(combined, yb.to(device))
        loss.backward()
        for name, param in model.named_parameters():
...
def ewc_loss_fn(model, old_params, fisher, device):
...
def lwf_loss_fn(model, old_model, x, device, temperature=2.0):
    old_model.eval()
    with torch.no_grad():
        old_out, _, _, _, _ = old_model(x.to(device))
    new_out, _, _, _, _ = model(x.to(device))
    return F.kl_div(
        F.log_softmax(new_out / temperature, dim=1),
        F.softmax(old_out / temperature, dim=1),
        reduction='batchmean'
    ) * (temperature ** 2)
...
parser.add_argument('--num_tasks', type=int, default=5,
                    help='Number of continual learning tasks (splits 100 classes)')
parser.add_argument('--pretrain_dataset', type=str, default='c100', choices=['c100', 'c10'],
                    help='Dataset for pre-differentiation: c100=CIFAR-100, c10=CIFAR-10')
parser.add_argument('--pretrain_split', type=int, default=0,
                    help='Pre-diff on first N classes only, CL on remaining 100-N classes (0=disabled)')
parser.add_argument('--cl_method', type=str, default='lifecycle',
                    choices=['lifecycle', 'ewc', 'lwf', 'finetune'],
                    help='Continual learning method')
parser.add_argument('--lambda_ewc', type=float, default=100.0,
...
# ── Determine num_classes for Phase 1 (pre-diff) ──
CLASS_OFFSET = args.pretrain_split  # 0=all classes, N=skip first N for CL
if args.pretrain_split > 0:
    pretrain_nclasses = args.pretrain_split
elif args.pretrain_dataset == 'c10':
    pretrain_nclasses = 10
else:
    pretrain_nclasses = NUM_CLASSES
...
model = ContinualLifecycleMoE(3, max_life=10, dominance_threshold=0.35,
                               ...
                               static_routing=args.static_routing,
                               num_classes=pretrain_nclasses).to(dev)
...
# ═══════════════════════════════════════════════════════════════
# Phase 1: Pre-differentiation
# ═══════════════════════════════════════════════════════════════
pretrain_done = (start_task > 0) or (start_epoch_in_task > 0)

if args.pretrain_epochs > 0 and not pretrain_done:
    pretrain_ds_name = 'CIFAR-10' if args.pretrain_dataset == 'c10' else (f'CIFAR-100 (classes 0-{args.pretrain_split-1})' if args.pretrain_split > 0 else 'CIFAR-100')
    print(f"\n{'═'*60}")
    print(f"PHASE 1: Pre-differentiation on {pretrain_ds_name}")
    print(f"  epochs={args.pretrain_epochs}  kill={args.pretrain_kill}  lambda_ar={args.pretrain_lambda_ar}")
    print(f"  output classes={pretrain_nclasses}")
    print(f"  Goal: break clone state (sim<0.85) before continual learning on CIFAR-100")
    print(f"{'═'*60}")
    ...
    if args.pretrain_dataset == 'c10':
        pretrain_full = datasets.CIFAR10(root='/root/data', train=True, download=True,
                                          transform=pretrain_transform)
        pretrain_full_test = datasets.CIFAR10(root='/root/data', train=False, download=True,
        ...
        pretrain_full_test = datasets.CIFAR100(root='/root/data', train=False, download=False,
                                                transform=pretrain_test_transform)
        if args.pretrain_split > 0:
            # Filter to first N classes only
            split_indices = [i for i, (_, y) in enumerate(pretrain_full) if y < args.pretrain_split]
            pretrain_full = Subset(pretrain_full, split_indices)
            split_test_indices = [i for i, (_, y) in enumerate(pretrain_full_test) if y < args.pretrain_split]
            pretrain_full_test = Subset(pretrain_full_test, split_test_indices)
    ...
    # ── Transition from pre-diff to continual learning ──
    if args.pretrain_dataset == 'c10' or args.pretrain_split > 0:
        # Cross-dataset or partial-class: rebuild model. Encoder+activations transfer; classifiers reinit
        print(f"  Rebuilding model: {pretrain_nclasses}→{NUM_CLASSES} classes...")
        encoder_sd = {k: v.clone() for k, v in model.encoder.state_dict().items()}
        activation_sds = []
        for i in range(NUM_EXPERTS):
            activation_sds.append({k: v.clone() for k, v in model.experts[i].activation.state_dict().items()})
        del model
        ...
        for i in range(NUM_EXPERTS):
            model.experts[i].activation.load_state_dict(activation_sds[i])
        # classifier heads reinitialized randomly (10→100, different shapes)
        opt = torch.optim.AdamW(model.parameters(), lr=1e-3, weight_decay=5e-4)
        sched = torch.optim.lr_scheduler.CosineAnnealingLR(opt, T_max=NUM_TASKS * EPOCHS_PER_TASK)
        print(f"  Rebuild done. Encoder+activations transferred, classifiers reinitialized.")
    else:
        # Same dataset: model already has 100-class output, keep opt state, reset scheduler
        sched = torch.optim.lr_scheduler.CosineAnnealingLR(opt, T_max=NUM_TASKS * EPOCHS_PER_TASK)

test_subset, _, _ = get_task_data(0, train=False, num_tasks=NUM_TASKS, class_offset=CLASS_OFFSET)  # for sim analysis

# ── CL method state ──
old_params = {}
fisher_accum = {}
old_model = None
is_cl_baseline = args.cl_method in ('ewc', 'lwf', 'finetune')

for task_id in range(start_task, NUM_TASKS):
    train_data, task_start_cls, _ = get_task_data(task_id, train=True, num_tasks=NUM_TASKS, class_offset=CLASS_OFFSET)
    train_loader = DataLoader(train_data, batch_size=BATCH_SIZE, shuffle=True, num_workers=2, pin_memory=True)

    print(f"\n{'─'*50}")
    task_nclasses = get_task_ranges(NUM_TASKS, CLASS_OFFSET)[task_id][2]
    print(f"TASK {task_id}: classes {task_start_cls}-{task_start_cls+task_nclasses-1}  method={args.cl_method}")
    print(f"{'─'*50}")

    # Task boundary
    if task_id > 0:
        if args.cl_method == 'lifecycle':
            ...
            # EWC/LwF/finetune: all parameters remain trainable
            if args.static_routing:
                model.set_static_expert(task_id % NUM_EXPERTS)
            trainable_params = sum(p.numel() for p in model.parameters() if p.requires_grad)
            print(f"  method={args.cl_method}, all params trainable ({trainable_params/1e6:.1f}M)")
    ...
    for epoch_in_task in range(start_ep, EPOCHS_PER_TASK):
        ...
        for xb, yb in train_loader:
            ...
            # CL baseline penalties
            if args.cl_method == 'ewc' and task_id > 0:
                ewc_loss = ewc_loss_fn(model, old_params, fisher_accum, dev)
                loss = loss + args.lambda_ewc * ewc_loss
            elif args.cl_method == 'lwf' and task_id > 0:
                lwf_loss = lwf_loss_fn(model, old_model, xb, dev, args.lwf_temperature)
                loss = loss + args.lambda_lwf * lwf_loss
    ...
    accs = evaluate_all_tasks(model, dev, num_tasks=NUM_TASKS, class_offset=CLASS_OFFSET)
    ...
    # Post-task: EWC/LwF state
    if args.cl_method == 'ewc':
        task_fisher = compute_fisher(model, train_loader, dev)
        for name in task_fisher:
            fisher_accum[name] = fisher_accum.get(name, 0) + task_fisher[name]
        old_params = save_old_params(model)
        print(f"  EWC: Fisher accumulated ({len(fisher_accum)} params)")
    elif args.cl_method == 'lwf':
        old_model = copy.deepcopy(model)
        old_model.eval()
        for p in old_model.parameters():
            p.requires_grad = False
        print(f"  LwF: old model snapshot saved")
