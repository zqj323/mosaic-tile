...
if args.pretrain_epochs > 0 and not pretrain_done:
    ...
    if args.pretrain_dataset == 'c10':
        ...
        else:        pretrain_loader = DataLoader(pretrain_full, batch_size=BATCH_SIZE, shuffle=True,
...
for task_id in range(start_task, NUM_TASKS):
    ...
    print(f"TASK {task_id}: classes {task_start_cls}-{task_start_cls+task_nclasses-1}  method={'lifecycle'}")
    print(f"{'─'*50}")
    ...
    if task_id > 0:
            ...
            if args.static_routing:
                model.set_static_expert(task_id % NUM_EXPERTS)
            trainable_params = sum(p.numel() for p in model.parameters() if p.requires_grad)
            print(f"  method={'lifecycle'}, all params trainable ({trainable_params/1e6:.1f}M)")
